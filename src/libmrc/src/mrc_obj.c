...
static void create_member_objs(MPI_Comm comm, void* p, struct param* descr)
{
...
static void destroy_member_objs(void* p, struct param* descr)
{
  for (int i = 0; descr && descr[i].name; i++) {
    if (descr[i].type == MRC_VAR_OBJ) {
      union param_u* pv = p + (unsigned long)descr[i].var;
...
static void set_from_options_member_objs(void* p, struct param* descr)
{
  for (int i = 0; descr && descr[i].name; i++) {
    if (descr[i].type == MRC_VAR_OBJ) {
      union param_u* pv = p + (unsigned long)descr[i].var;
...
static void view_member_objs(void* p, struct param* descr)
{
  for (int i = 0; descr && descr[i].name; i++) {
    if (descr[i].type == MRC_VAR_OBJ) {
      union param_u* pv = p + (unsigned long)descr[i].var;
...
static void setup_member_objs(void* p, struct param* descr)
{
  for (int i = 0; descr && descr[i].name; i++) {
    if (descr[i].type == MRC_VAR_OBJ) {
      union param_u* pv = p + (unsigned long)descr[i].var;
...
static void read_member_objs(struct mrc_io* io, struct mrc_obj* obj, void* p,
                             struct param* descr)
{
  for (int i = 0; descr && descr[i].name; i++) {
    if (descr[i].type == MRC_VAR_OBJ) {
      union param_u* pv = p + (unsigned long)descr[i].var;
      pv->u_obj =
        __mrc_io_read_ref(io, obj, descr[i].name, descr[i].u.mrc_obj.cls);
    }
  }
}

static struct mrc_obj* obj_create(MPI_Comm comm, struct mrc_class* cls,
                                  bool basic_only)
{
...
struct mrc_obj_ops* mrc_obj_find_subclass_ops(struct mrc_class* cls,
                                              const char* subclass)
{
  if (!subclass)
    return NULL;
...
static void init_class(struct mrc_class* cls)
{
  if (!cls->initialized) {
...
#pragma omp critical
  ...
  while (!list_empty(&obj->children_list)) {
    struct mrc_obj* child =
      list_entry(obj->children_list.next, struct mrc_obj, child_entry);
...
static void obj_set_type(struct mrc_obj* obj, const char* subclass,
                         bool basic_only)
{
  /* if (obj->ops) { */
...
void mrc_obj_set_type(struct mrc_obj* obj, const char* subclass)
{
  obj_set_type(obj, subclass, false);
}

static void mrc_obj_set_from_options_this(struct mrc_obj* obj)
{
  struct mrc_class* cls = obj->cls;
  ...
  if (cls->param_descr) {
    char* p = (char*)obj + cls->param_offset;
    mrc_params_parse_nodefault(p, cls->param_descr, mrc_obj_name(obj),
                               obj->comm);
  ...
  if (obj->ops) {
    if (obj->ops->param_descr) {
      char* p = (char*)obj->subctx + obj->ops->param_offset;
      mrc_params_parse_nodefault(p, obj->ops->param_descr, obj->ops->name,
                                 obj->comm);
      mrc_params_parse_pfx(p, obj->ops->param_descr, mrc_obj_name(obj),
                           obj->comm);
    }
...
void mrc_obj_set_param_type(struct mrc_obj* obj, const char* name, int type,
                            union param_u* uval)
{
  struct mrc_class* cls = obj->cls;
  if (cls->param_descr) {
...
int mrc_obj_get_param_type(struct mrc_obj* obj, const char* name, int type,
                           union param_u* uval)
{
  struct mrc_class* cls = obj->cls;
  if (cls->param_descr) {
...
void mrc_obj_set_param_string(struct mrc_obj* obj, const char* name,
                              const char* val)
{
...
void mrc_obj_set_param_int3(struct mrc_obj* obj, const char* name,
                            const int val[3])
{
...
void mrc_obj_set_param_float3(struct mrc_obj* obj, const char* name,
                              const float val[3])
{
...
void mrc_obj_set_param_double3(struct mrc_obj* obj, const char* name,
                               const double val[3])
{
...
void mrc_obj_set_param_int_array(struct mrc_obj* obj, const char* name,
...
void mrc_obj_set_param_float_array(struct mrc_obj* obj, const char* name,
...
int mrc_obj_get_param_double(struct mrc_obj* obj, const char* name,
                             double* pval)
{
  union param_u uval;
...
int mrc_obj_get_param_string(struct mrc_obj* obj, const char* name,
                             const char** val)
{
  union param_u uval;
...
int mrc_obj_get_param_double3(struct mrc_obj* obj, const char* name,
                              double* pval)
{
  union param_u uval;
...
int mrc_obj_get_param_obj(struct mrc_obj* obj, const char* name,
                          struct mrc_obj** pval)
{
  union param_u uval;
...
int mrc_obj_get_param_ptr(struct mrc_obj* obj, const char* name, void** val)
{
  union param_u uval;
...
int mrc_obj_get_param_float_array_nr_vals(struct mrc_obj* obj, const char* name,
                                          int* nr_vals)
{
  union param_u uval;
...
int mrc_obj_get_param_float_array(struct mrc_obj* obj, const char* name,
                                  float* pval)
{
  union param_u uval;
  int err = mrc_obj_get_param_type(obj, name, PT_FLOAT_ARRAY, &uval);
...
static void mrc_obj_view_this(struct mrc_obj* obj)
{
  struct mrc_class* cls = obj->cls;
...
static void mrc_obj_setup_default(struct mrc_obj* obj)
{
...
void mrc_obj_read_super(struct mrc_obj* obj, struct mrc_io* io)
{
  ...
  if (cls->read) {
    ...
    // FIXME, ugly: basically the same as mrc_obj_setup(), but skipping the
    // children setup
    obj->is_setup = true;
...
static void mrc_obj_read_params(struct mrc_obj* obj, void* p,
                                struct param* params, const char* path,
                                struct mrc_io* io)
{
...
static void mrc_obj_read_dict(struct mrc_obj* obj, const char* path,
                              struct mrc_io* io)
{
  int cnt;
  ...
  for (int i = 0; i < cnt; i++) {
    ...
    if (type == PT_OBJ || type == MRC_VAR_OBJ) {
      mpi_printf(mrc_io_comm(io),
                 "!!! WARNING: cannot read back dictionary object %s (type %d) "
                 "in %s!!!\n",
...
static void mrc_obj_read2(struct mrc_obj* obj, struct mrc_io* io,
                          const char* path)
{
  struct mrc_class* cls = obj->cls;
...
struct mrc_obj* mrc_obj_read(struct mrc_io* io, const char* path,
...
struct mrc_obj* mrc_obj_read_comm(struct mrc_io* io, const char* path,
                                  struct mrc_class* cls, MPI_Comm comm)
{
  init_class(cls);
...
static void mrc_obj_write_params(struct mrc_obj* obj, void* p,
                                 struct param* params, const char* path,
                                 struct mrc_io* io)
{
  for (int i = 0; params[i].name; i++) {
    ...
    if (prm->type == PT_OBJ || prm->type == MRC_VAR_OBJ) {
...
static void mrc_obj_write_dict(struct mrc_obj* obj, const char* path,
                               struct mrc_io* io)
{
  int cnt = 0;
  struct mrc_dict_entry* e;
  ...
  __list_for_each_entry(e, &obj->dict_list, entry, struct mrc_dict_entry)
  {
    if (e->prm.type == PT_OBJ || e->prm.type == MRC_VAR_OBJ) {
...
static unsigned long mrc_obj_uid(struct mrc_obj* obj)
{
...
void mrc_obj_dict_add_string(struct mrc_obj* obj, const char* name,
                             const char* val)
{
  union param_u uval;
  uval.u_string = val;
...
void mrc_obj_dict_add_obj(struct mrc_obj* obj, const char* name,
                          struct mrc_obj* val)
{
  union param_u uval;
  uval.u_obj = val;
...
void mrc_obj_dict_add_float_array(struct mrc_obj* obj, const char* name,
...
static void get_var(void* p, struct param* descr, const char* name, int type,
                    union param_u** pv)
{
  for (int i = 0; descr && descr[i].name; i++) {
...
static int mrc_obj_get_var_type(struct mrc_obj* obj, const char* name, int type,
                                union param_u** pv)
{
