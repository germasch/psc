...
Grid_t* setupGrid()
{
  // --- setup domain
#if CASE == CASE_3D
  Grid_t::Real3 LL = {800., 800., 3. * 800.};   // domain size (in d_e)
  Int3 gdims = {2 * 768, 2 * 768, 3 * 2 * 768}; // global number of grid points
  Int3 np = {2 * 24, 2 * 24, 3 * 2 * 24};       // division into patches
#elif CASE == CASE_2D
