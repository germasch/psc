...
void cuda_mfields::copy_comp_yz(int ym, cuda_mfields* cmflds_x, int xm)
{
  ...
  for (int p = 0; p < n_patches(); p++) {
    k_copy_comp_yz<<<dimGrid, dimBlock>>>((*this)[p].storage().data(), ym,
                                          (*cmflds_x)[p].storage().data(), xm, my, mz);
  }
...
void cuda_mfields::copy_comp_xyz(int ym, cuda_mfields* cmflds_x, int xm)
{
  ...
  for (int p = 0; p < n_patches(); p++) {
    k_copy_comp_xyz<<<dimGrid, dimBlock>>>(
      (*this)[p].storage().data(), ym, (*cmflds_x)[p].storage().data(), xm, mx, my, mz);
  }
  cuda_sync_if_enabled();
}
                                 ...
                                 int xm)
{
  ...
  for (int p = 0; p < n_patches(); p++) {
    k_axpy_comp_xyz<<<dimGrid, dimBlock>>>(
      (*this)[p].storage().data(), ym, a, (*cmflds_x)[p].storage().data(), xm, mx, my, mz);
  }
  cuda_sync_if_enabled();
}
...
void cuda_mfields::zero_comp(int m, dim_yz tag)
{
  ...
  for (int p = 0; p < n_patches(); p++) {
    k_zero_comp_yz<<<dimGrid, dimBlock>>>((*this)[p].storage().data(), m, my, mz);
  }
  cuda_sync_if_enabled();
}
