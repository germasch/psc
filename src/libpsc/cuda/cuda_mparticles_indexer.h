...
struct DParticleIndexer
{
  ...
  __device__ int validCellIndex(const DParticleCuda& prt, int p) const
  {
    ...
    uint pos_y = __float2int_rd(prt.x[1] * dxi_[1]);
    uint pos_z = __float2int_rd(prt.x[2] * dxi_[2]);
  ...
  __device__ int blockIndex(const DParticleCuda& prt, int p) const
  {
    int block_pos[3] = {int(__float2int_rd(prt.x[0] * dxi_[0]) / BS::x::value),
                        ...
                        int(__float2int_rd(prt.x[2] * dxi_[2]) / BS::z::value)};
  ...
  __device__ uint n_blocks() const { return n_blocks_; }

  __device__ const uint* b_mx() const { return b_mx_; }
...
struct BlockSimple2 : BlockBase
{
  ...
  static int n_concurrent_blocks(CudaMparticles& cmprts)
  {
    ...
    int n_blocks =
      n_blocks_per_patch >= MAX_N_BLOCKS ? 1 : MAX_N_BLOCKS / n_blocks_per_patch;
    return n_blocks;
  }
