...
template <typename T, bool UNGUARDED_IO, int SETS_PER_PASS, typename PreprocessFunctor>
__device__ __forceinline__ void ReadSetsV(
	int block_offset,
	typename VecType<T, 2>::Type pairs[SETS_PER_PASS],
	const int BASE2,
	int extra[1],
	PreprocessFunctor preprocess = PreprocessFunctor())				
{
  // N.B. -- I wish we could do some pragma unrolling here too, but the compiler makes it 1% slower
  if (SETS_PER_PASS > 0) {
    pairs[0].x = block_offset + 2*(threadIdx.x + BASE2 + (B40C_RADIXSORT_THREADS * 0));
    pairs[0].y = block_offset + 2*(threadIdx.x + BASE2 + (B40C_RADIXSORT_THREADS * 0)) + 1;
  }
  if (SETS_PER_PASS > 1) {
    pairs[1].x = block_offset + 2*(threadIdx.x + BASE2 + (B40C_RADIXSORT_THREADS * 1));
    pairs[1].y = block_offset + 2*(threadIdx.x + BASE2 + (B40C_RADIXSORT_THREADS * 1)) + 1;
  }
  if (SETS_PER_PASS > 2) {
    pairs[2].x = block_offset + 2*(threadIdx.x + BASE2 + (B40C_RADIXSORT_THREADS * 2));
    pairs[2].y = block_offset + 2*(threadIdx.x + BASE2 + (B40C_RADIXSORT_THREADS * 2)) + 1;
  }
  if (SETS_PER_PASS > 3) {
    pairs[3].x = block_offset + 2*(threadIdx.x + BASE2 + (B40C_RADIXSORT_THREADS * 3));
    pairs[3].y = block_offset + 2*(threadIdx.x + BASE2 + (B40C_RADIXSORT_THREADS * 3)) + 1;
  }
	...
	int extra[1])				
{
	...
	if (!IsKeysOnly<V>()) {
		...
		if (PASSES_PER_CYCLE > 0) ReadSetsV<V, UNGUARDED_IO, SETS_PER_PASS, NopFunctor<V> >(block_offset, datapairs[0], B40C_RADIXSORT_THREADS * SETS_PER_PASS * 0, extra);
		if (PASSES_PER_CYCLE > 1) ReadSetsV<V, UNGUARDED_IO, SETS_PER_PASS, NopFunctor<V> >(block_offset, datapairs[1], B40C_RADIXSORT_THREADS * SETS_PER_PASS * 1, extra);
	...
	unsigned int nr_total_blocks)
{
	...
	_B40C_REG_MISER_QUALIFIER_ int extra0[1];
	_B40C_REG_MISER_QUALIFIER_ int extra[1];
	_B40C_REG_MISER_QUALIFIER_ int oob[1];

	// calculate our threadblock's range
	int n_elements = B40C_RADIXSORT_CYCLE_ELEMENTS(__CUDA_ARCH__, K, V);
	int block_offset = d_offsets[blockIdx.x];
	int block_offset_end = d_offsets[blockIdx.x + 1];
	int block_offset_next_aligned = (block_offset + n_elements - 1) & ~ (n_elements - 1);
	if (block_offset_end <= block_offset_next_aligned) {
	  extra[0] = 0;
	  oob[0] = 0;
	  extra0[0] = block_offset_end - block_offset;
	} else {
	  extra0[0] = block_offset_next_aligned - block_offset;
	  int cycles = (block_offset_end - block_offset_next_aligned) / n_elements;
	  oob[0] = block_offset_next_aligned + cycles * n_elements;	// out-of-bounds
	  extra[0] = block_offset_end - oob[0];
	}
	...
	// Sync to acquire from_temp_storage
	__syncthreads();
